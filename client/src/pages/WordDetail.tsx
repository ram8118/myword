...
export default function WordDetail() {
  ...
  async function handlePronounce() {
    ...
    speak.mutate(
      { text: target },
      {
        onSuccess: (data) => setTtsPayload(data),
        onError: (e) =>
          toast({
      ...
      <div className="mt-5">
        {savedWord.isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {ttsPayload ? (
              <GlassCard className="p-5" data-testid="detail-tts-card">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-bold text-foreground">Pronunciation audio</div>
                    <div className="text-xs text-muted-foreground">For “{savedWord.data.word}”.</div>
                  </div>
                  <button
                    onClick={() => setTtsPayload(null)}
                    data-testid="detail-tts-dismiss"
                ...
                <div className="mt-3">
                  <AudioPlayer audioBase64={ttsPayload.audioBase64} contentType={ttsPayload.contentType} autoPlay />
                </div>
              </GlassCard>
            ) : null}
          </div>
